...
function renderUnsubscribePage(detail: string, success: boolean): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
async function handleUnsubscribe(req: Request): Promise<Response> {
  ...
  if (!email) {
    return new Response(renderUnsubscribePage("Missing email parameter.", false), {
      status: 400,
  ...
  if (error) {
    ...
    return new Response(renderUnsubscribePage("Something went wrong. Please try again.", false), {
      status: 500,
      headers: { "Content-Type": "text/html", ...corsHeaders },
    });
  }
  ...
  return new Response(renderUnsubscribePage(email, true), {
    status: 200,
    headers: { "Content-Type": "text/html", ...corsHeaders },
  });
}
...
const handler = async (req: Request): Promise<Response> => {
  ...
  // GET requests = unsubscribe handler (no auth needed)
  if (req.method === "GET") {
  ...
  try {
    ...
    const unsubscribeUrl = `${supabaseUrl}/functions/v1/send-quotation-email?email=${encodeURIComponent(to)}`;
